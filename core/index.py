...
def search_index(
    ...
    *,
    ...
    # Take a bit more than top_k and then filter down
    internal_k = max(top_k * 4, 20)
    ...
    for idx, score in zip(ids[0], scores[0]):
        ...
        # Remove obvious noise chunks
        if _is_obviously_noise(ch.text):
            continue

        # Light keyword filtering for better precision
        if keyword_filter and not _chunk_has_keywords(ch.text, keywords, min_hits=1):
            # still allow if title/url looks very relevant
